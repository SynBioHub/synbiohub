...
class TestSubmit(TestCase):
    ...
    def test_submit(self):
        ...
        compare_get_request(
            "/", test_name="after_admin_login", headers=headers)
...
#        self.create_collection2()
        ...
        compare_get_request('/user/:userId/:collectionId/:displayId/:version/removeCollection',
                            route_parameters=["testuser", "testid", "testid_collection", "1"])
        compare_get_request('/user/:userId/:collectionId/:displayId/:version/removeCollection',
                            route_parameters=["testuser", "testid2", "testid2_collection", "1"], test_name='remove_second')
...
#        """
        ...
        # get the view
        compare_get_request("/user/:userId/:collectionId/:displayId/:version/makePublic",
                            route_parameters=["testuser", "testid0", "testid0_collection", "1"])

        data['tabState'] = 'new'

        # make the collection public
        compare_post_request("/user/:userId/:collectionId/:displayId/:version/makePublic",
                             route_parameters=["testuser", "testid1", "testid1_collection", "1"], data=data)

        # try to delete the collection
        with self.assertRaises(requests.exceptions.HTTPError):
            compare_get_request("/public/:collectionId/:displayId/:version/removeCollection",
                                route_parameters=["testid1", "testid1_collection", "1"], test_name='remove')
        ...
        compare_post_request("submit", data, headers={
                             "Accept": "text/plain"}, files=files, test_name="collection_for_test_attachment")
        ...
        files = {'file': ("./SBOLTestRunner/src/main/resources/SBOLTestSuite/SBOL2/Measure.xml",
                          open('./SBOLTestRunner/src/main/resources/SBOLTestSuite/SBOL2/Measure.xml', 'rb'))}

        compare_post_request("submit", data, headers={
                             "Accept": "text/plain"}, files=files, test_name="collection_for_test_hash")
