...
function getAttachmentsFromTopLevel(sbol, topLevel, share) {
    ...
    topLevel.getAnnotations('http://sbols.org/v2#Attachment').map((attachmentURI) => {
        console.log("SBOL Attachment URI:" + attachmentURI)

        const attachment = sbol.lookupURI(attachmentURI)

        if(attachment && !(attachment instanceof URI)) {

            const size = parseInt(attachment.getAnnotation('http://sbols.org/v2#size') || '-1')
	    ...
	    if (attachment.uri.toString().startsWith(config.get('databasePrefix')+'user/') && share) {
            ...
            attachments.push({
                name: attachment.name,
                type: config.get('attachmentTypeToTypeName')[attachment.getAnnotation('http://sbols.org/v2#size')] || 'Other',
                url: url,
                size: size,
                sizeString: size == -1 ? null : filesize(size)
            })
