    ...
    attachmentList.map((attachmentURI) => {
      ...
      return fetchSBOLObjectRecursive(uri, graphUri).then((result) => {
        ...
        if (attachment) {
          ...
          var source = '/' + attachment.source.toString().replace(config.get('databasePrefix'), '')
          if (attachment.source.toString().startsWith(config.get('databasePrefix') + 'user/') && share) {
            source += '/' + sha1('synbiohub_' + sha1(attachment.source.toString()) + config.get('shareLinkSalt')) + '/share'
          }
