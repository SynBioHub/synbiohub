...
// console.log('Adding:'+name+' to:'+topLevelUri)
  const displayId = 'attachment_' + uuid().replace('-', '')
  ...
  topLevel.attachments.map(attachment => {
    if (attachment) {
      if (!(attachment instanceof URI)) {
        ...
        var attachmentType = attachment.format.toString().replace('http://identifiers.org/combine.specifications/', '').replace('http://identifiers.org/edam/', '').replace('http://purl.org/NET/mediatypes/application/', '').replace('http://purl.org/NET/mediatypes/audio/', '').replace('http://purl.org/NET/mediatypes/font/', '').replace('http://purl.org/NET/mediatypes/example/', '').replace('http://purl.org/NET/mediatypes/image/', '').replace('http://purl.org/NET/mediatypes/message/', '').replace('http://purl.org/NET/mediatypes/model/', '').replace('http://purl.org/NET/mediatypes/multipart/', '').replace('http://purl.org/NET/mediatypes/text/', '').replace('http://purl.org/NET/mediatypes/video/', '')
    ...
    attachmentList.map((attachmentURI) => {
      ...
      return fetchSBOLObjectRecursive(uri, graphUri).then((result) => {
        ...
        if (attachment) {
          ...
          format = format.toString().replace('http://identifiers.org/combine.specifications/', '').replace('http://identifiers.org/edam/', '').replace('http://purl.org/NET/mediatypes/application/', '').replace('http://purl.org/NET/mediatypes/audio/', '').replace('http://purl.org/NET/mediatypes/font/', '').replace('http://purl.org/NET/mediatypes/example/', '').replace('http://purl.org/NET/mediatypes/image/', '').replace('http://purl.org/NET/mediatypes/message/', '').replace('http://purl.org/NET/mediatypes/model/', '').replace('http://purl.org/NET/mediatypes/multipart/', '').replace('http://purl.org/NET/mediatypes/text/', '').replace('http://purl.org/NET/mediatypes/video/', '')
