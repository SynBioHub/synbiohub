        ...
        attachmentList.map((attachmentURI) => {
            ...
            return fetchSBOLObjectRecursive(uri, graphUri).then((result) => {
                ...
                if (attachment) {
                    ...
                    const size = parseInt(attachment.getAnnotation('http://wiki.synbiohub.org/wiki/Terms/synbiohub#attachmentSize') || sbol.size || '-1')
                    ...
                    return Promise.resolve({
                        name: attachment.name,
                        type: config.get('attachmentTypeToTypeName')[attachment.getAnnotation('http://wiki.synbiohub.org/wiki/Terms/synbiohub#attachmentType')] || attachment.type || 'Other',
                        url: url,
                        size: size,
                        sizeString: size == -1 ? null : filesize(size)
                    })
