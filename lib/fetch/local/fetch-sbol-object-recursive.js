      ...
      function fetchNonLocalSBOL (next) {
        async.each(sbol.unresolvedURIs, (uri, nextUri) => {
          ...
          if (uri.toString().startsWith('http://wiki.synbiohub.org/')) {
            ...
            if (!uri.toString().startsWith(databasePrefix) && webOfRegistries[prefix]) {
              ...
              request({
                ...
                if (err || response.statusCode >= 300) {
                  if (response) {
                    console.log('ERROR ' + response.statusCode + ' fetching non-local:' + uri)
                  } else {
                    console.log('ERROR fetching non-local:' + uri)
                  }
                  nextUri()
                } else {
                  if (!body.toString().startsWith('<!DOCTYPE html><')) {
                    console.log('Received non-local:' + uri)
                    sbol.loadRDF(body, nextUri)
                  } else {
                    console.log('HTML returned when fetching non-local:' + uri)
                    nextUri()
                  }
                }
              })
