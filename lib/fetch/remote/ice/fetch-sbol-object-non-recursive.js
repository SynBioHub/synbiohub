...
const Cut = require('sboljs/lib/Cut')
...
function fetchSBOLObjectNonRecursive (remoteConfig, sbol, type, uri) {
  ...
  if (displayId === remoteConfig.rootCollection.displayId) {
    ...
    rootCollection.persistentIdentity = config.get('databasePrefix') + 'public/' + remoteConfig.id + '/' + rootCollection.displayId
    rootCollection.uri = rootCollection.persistentIdentity + '/' + rootCollection.version
  ...
  if (displayId === 'available') {
    return availableFolderToCollection('available').then((collection) => {
      return Promise.resolve({
        sbol: sbol,
        object: collection,
        remote: true
  ...
  function availableFolderToCollection (folderId) {
    return Promise.all([
      ice.getRootFolderEntries(remoteConfig)
    ]).then((res) => {
      const [ entries ] = res
  ...
  function folderToCollection (folderId) {
    return Promise.all([
      ...
      if (folder.creationTime) { collection.addStringAnnotation('http://purl.org/dc/terms/created', folder.creationTime + '') }

      if (folder.modificationTime) { collection.addStringAnnotation('http://purl.org/dc/terms/modified', folder.modificationTime + '') }
...
*             "strainData": { "host": "DH10B", "genotypePhenotype": "" } } */
      ...
      if (part.longDescription) { componentDefinition.addStringAnnotation('http://wiki.synbiohub.org/wiki/Terms/synbiohub#mutableDescription', part.longDescription + '') }
      ...
      if (part.intellectualProperty) { componentDefinition.addStringAnnotation('http://wiki.synbiohub.org/wiki/Terms/ice#intellectualProperty', part.intellectualProperty + '') }
      ...
      if (part.fundingSource) { componentDefinition.addStringAnnotation('http://wiki.synbiohub.org/wiki/Terms/ice#fundingSource', part.fundingSource + '') }
