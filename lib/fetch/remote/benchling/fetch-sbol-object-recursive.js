...
function fetchSBOLObjectRecursive(remoteConfig, sbol, type, uri) {
    ...
    function folderToCollection(folderId) {
        ...
        return Promise.all([
            ...
            collection.wasDerivedFrom = remoteConfig.url + '/folders/' + folder.id
...
*/
		...
		const range = sbol.range()
                ...
                range.start = annotation.start<annotation.end?annotation.start:annotation.end
		// TODO: This is ugly, but not sure how else to fix
		if (range.start===0) {
		    range.start = 1
		}
                range.end = annotation.end>annotation.start?annotation.end:annotation.start
		// TODO: This is ugly, but not sure how else to fix
		if (range.end===0) {
		    range.end = 1
		}
