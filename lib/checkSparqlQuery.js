...
function checkQuery (query, user, baseUri) {
...
graph === config.get('triplestore').defaultGraph || (baseUri && baseUri.startsWith(graph))
    }
...
// throw new Error('unknown query type: ' + node.queryType)
  ...
  function checkServiceNode (node) {
    ...
    function checkWor (url) {
      for (var i = 0; i < wor.length; ++i) {
        if (url.indexOf(wor[i]) === 0) { return true }
      }
  ...
  function checkBgpNode (node) {
    node.triples.forEach(triple => {
      if (!triple.subject.startsWith('?') && !triple.subject.startsWith(userGraph) &&
          !triple.subject.startsWith(config.get('triplestore').defaultGraph) &&
          (!baseUri || !triple.subject.startsWith(baseUri))) {
        throw new Error('Cannot use subjects that access other users\' graphs.')
      }
    })
  }
}
