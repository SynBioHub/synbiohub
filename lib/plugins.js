...
function buildSubmissionManifest (filename) {
  ...
  let exposeUrl
  if (config.get('pluginsUseLocalCompose')) {
    exposeUrl = `${config.get('pluginLocalComposePrefix')}expose/${exposeId}`
  } else {
    exposeUrl = `${instanceUrl}expose/${exposeId}`
  }
...
async function runSubmitPlugin (plugin, filename) {
  ...
  let runParams = {
    manifest: manifest,
    instanceUrl: config.get('pluginsUseLocalCompose') ? config.get('pluginLocalComposePrefix') : config.get('instanceUrl')
  }
...
function runVisualPlugin (plugin, body, token) {
  let instanceData = {
    instanceUrl: config.get('pluginsUseLocalCompose') ? config.get('pluginLocalComposePrefix') : config.get('instanceUrl')
  }
  ...
  return runPlugin(plugin, body, token)
}

function runDownloadPlugin (plugin, body, token) {
  let instanceData = {
    instanceUrl: config.get('pluginsUseLocalCompose') ? config.get('pluginLocalComposePrefix') : config.get('instanceUrl')
  }
...
function runPlugin (plugin, body, token) {
