...
function advancedSearchForm (req, res, properties, submissionData, locals) {
  ...
  getUriList(null).then((lists) => {
    ...
    locals = extend({
      ...
      annotations: lists.annotationList,
  ...
  getUriList(null).then((lists) => {
    ...
    if (req.body.type !== 'Any Type') {
      if (req.body.type.toString().startsWith('http')) {
        criteriaStr += '   ?subject sbol2:type <' + req.body.type + '> . '
    ...
    if (req.body.role !== 'Any Role') {
      if (req.body.role.toString().startsWith('http')) {
        criteriaStr += '   ?subject sbol2:role <' + req.body.partType + '> . '
        query += 'role=<' + req.body.partType + '>&'
...
'   <' + collNS + collections[collectionIndex] + '> sbol2:member ?subject .'
        query += 'collection=<' + collNS + collections[collectionIndex] + '>&'
      }
    }
...
function getUriList (graphUri) {
  ...
  var typeQuery = 'PREFIX sbol2: <http://sbols.org/v2#> SELECT DISTINCT ?object WHERE { ?tl a sbol2:ComponentDefinition .  ?tl sbol2:type ?object }'
  ...
  var roleQuery = 'PREFIX sbol2: <http://sbols.org/v2#> SELECT DISTINCT ?object WHERE { ?tl a sbol2:ComponentDefinition .  ?tl sbol2:role ?object }'
  ...
  var annotationQuery = 'select distinct ?predicate where { ?subject ?predicate ?object }'
  ...
  return Promise.all([
    ...
    sparql.queryJson(annotationQuery, graphUri).then((annotationList) => {
      annotationList.forEach((result) => {
        result.uri = result.predicate
        if (result.uri.toString().startsWith(igemNS)) {
          result.name = result.uri.replace(igemNS, 'igem:')
        } else if (result.uri.toString().startsWith(provNS)) {
          result.name = result.uri.replace(provNS, 'prov:')
        } else if (result.uri.toString().startsWith(sbolNS)) {
          result.name = result.uri.replace(sbolNS, '')
        } else if (result.uri.toString().startsWith(sbhNS)) {
          result.name = result.uri.replace(sbhNS, 'sbh:')
        } else if (result.uri.toString().startsWith(celloNS)) {
          result.name = result.uri.replace(celloNS, 'cello:')
        } else if (result.uri.toString().startsWith(dctermsNS)) {
          result.name = result.uri.replace(dctermsNS, 'dcterms:')
        } else if (result.uri.toString().startsWith(dcNS)) {
          result.name = result.uri.replace(dctermsNS, 'dc:')
        } else {
          result.name = result.uri
        }
        console.log(result.name)
        result.name = result.predicate
        delete result.predicate
      })

      annotationList.sort(sortByNames)

      return Promise.resolve({ annotationList: annotationList })
    }),
