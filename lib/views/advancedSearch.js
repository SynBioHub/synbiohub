...
function getUriList(store, callback) {
    ...
    var sampleStatusQuery = 'PREFIX igem: <http://synbiohub.org/terms/igem/> SELECT DISTINCT ?object WHERE { ?tl igem:sampleStatus ?object }'
    ...
    async.parallel({
       one: function getCreatorList(next) {
        ...
        twoB: function getSampleStatusList(next) {
	    store.sparql(sampleStatusQuery, (err, sampleStatusList) => {
		if(err) {
		    callback(err)
		} else {
		    sampleStatusList.map((result) => {
			result.uri = result.object
			delete result.object
			result.name = result.uri
			return result
		    })	
		    sampleStatusList.sort(function(a,b){ if (a.name < b.name) { return -1 } else { return 1 } })
		    next(null, sampleStatusList)
		}
	...
	callback(null, results.one, results.twoA, results.twoB, results.two, results.three, results.four, results.five, results.six, results.seven)
    })
}
...
function advancedSearchForm(req, res, properties, submissionData, locals) {
  ...
  var submissionID = '';
    ...
    getUriList(stack.getDefaultStore(), (err, creatorList, partStatusList, sampleStatusList, statusList, resultList, typeList, roleList, igemRoleList, collectionList) => {
	...
	locals = extend({
	    ...
	    creators: creatorList,
            ...
            sampleStatuses: sampleStatusList,
            statuses: statusList,
            results: resultList,
	    types: typeList,
	    roles: roleList,
...
function advancedSearchPost(req, res) {
	...
	if(req.body.sampleStatus!=='Any iGEM Sample Status') {
	    criteriaStr += '   ?subject igem:sampleStatus \'' + req.body.sampleStatus + '\' . '
	    query += 'igem:sampleStatus=\''+req.body.sampleStatus+'\'&'
	}
    ...
    search(null, criteria, req.query.offset, limit, (err, count, results) => {
