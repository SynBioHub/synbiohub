...
module.exports = function (req, res) {
  if (req.method === 'POST') {
    if (req.body.adv === 'Search' || req.originalUrl.endsWith('/createCollection')) {
...
function getQuery (searchPred, searchObj, filterObj, criteriaStr, queryStr, list) {
  ...
  if (searchPred !== 'No Filter' && searchObj !== 'No Filter' && filterObj !== 'No Filter') {
    ...
    var objStr
    if (searchObj) {
      objStr = longName(searchObj)
    } else {
      var mappedList = {}
      for (var i = 0; i < list.length; i++) {
        mappedList[list[i].name] = list[i]
      }
      console.log(JSON.stringify(mappedList))
