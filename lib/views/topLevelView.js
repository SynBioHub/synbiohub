...
module.exports = function (req, res, type) {
  typeShortName = getShortName(type)
...
'}'
    ...
    let plugins = config.get('plugins').rendering.map(plugin => {
      let safeName = plugin.name.replace(/[\W_]+/g, '_')
      let runUrl = plugin.url + 'run'

      let pluginRequest = post(runUrl, {
        json: true,
        body: data
      })

      let streamId = stream.create(pluginRequest)
...
typeShortName === 'Sequence' || typeShortName === 'CombinatorialDerivation' ||
typeShortName === 'ComponentInstance' || typeShortName === 'FunctionalComponent' ||
...
typeShortName === 'GenericLocation' || typeShortName === 'ModuleInstance' ||
