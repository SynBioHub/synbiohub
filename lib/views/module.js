...
module.exports = function(req, res) {
    ...
    async.series([
		...
		moduleDefinition.description = wiky.process(moduleDefinition.description, {})
	    }

	    moduleDefinition.url = '/' + moduleDefinition.uri.toString().replace(config.get('databasePrefix'),'')

            locals.meta = moduleDefinition

            if(req.params.userId) {
                locals.sbolUrl = '/user/' + encodeURIComponent(req.params.userId) + '/' + designId + '/' + moduleDefinition.displayId + '.xml'
                locals.fastaUrl = '/user/' + encodeURIComponent(req.params.userId) + '/' + designId + '/' + moduleDefinition.displayId + '.fasta'
                locals.genBankUrl = '/user/' + encodeURIComponent(req.params.userId) + '/' + designId + '/' + moduleDefinition.displayId + '.gb'
                ...
                locals.fastaUrl = '/public/' + designId + '/' + moduleDefinition.displayId + '.fasta'
                locals.genBankUrl = '/public/' + designId + '/' + moduleDefinition.displayId + '.gb'
            ...
            res.send(pug.renderFile('templates/views/module.jade', locals))
        }
    ], function done(err) {
