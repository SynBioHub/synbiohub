...
module.exports = function(req, res) {
    ...
    async.series([
		...
		console.log(model)
		model.url = '/' + model.toString().replace(config.get('databasePrefix'),'')
		model.version = model.toString().substring(model.toString().lastIndexOf('/')+1)
		var persId = model.toString().substring(0,model.toString().lastIndexOf('/'))
