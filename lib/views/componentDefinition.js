...
module.exports = function(req, res) {
    ...
    async.series([
	    ...
	    // if (collectionId === 'igem') {

	    // 	var query = 'PREFIX sbol2: <http://sbols.org/v2#>\n' +
	    // 	    'select ?s where {\n' +
	    // 	    '?s a sbol2:ComponentDefinition .\n'+
	    // 	    '?s sbol2:displayId \''+ req.params.displayId + '\' .\n' +
	    // 	    '?s sbol2:version \'' + req.params.version + '\'\n}\n'
	    ...
	    // 		    uri = results[0].s
	    // 		    designId = uri.replace('http://synbiohub.org/public/','')
	    // 		} else {
        ...
        function retrieveSBOL(next) {
            ...
            getComponentDefinition(uri, graphUris, function(err, sbol, _componentDefinition) {
                ...
                if(err) {
                    ...
                    if(!meta) {
                        return res.status(500).send('summarizeComponentDefinition returned null\n')
                    }
        ...
        function lookupEncodedProteins(next) {
            ...
            sparql.queryJson(query, componentDefinition.graphUri, (err, results) => {
    ...
    //<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:prov="http://www.w3.org/ns/prov#" xmlns:sbol="http://sbols.org/v2#" xmlns:sybio="http://www.sybio.ncl.ac.uk#" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:ncbi="http://www.ncbi.nlm.nih.gov#">
                ...
                getComponentDefinitionMetadata(uri, stores, (err, metadata) => {
		...
		if (type.description.name === 'DnaRegion') isDNA = 1
