...
module.exports = function(req, res) {
	...
	criteria.push(
		...
		'   ?collection sbol2:member ?subject .' +
                '   FILTER(?collection = ' + '<' + base64.decode(req.params.collectionURI) + '>)'
	)
    }

    if(req.params.roleURI) {
	criteria.push(
	    '   ?subject sbol2:role ?role .' +
		'   FILTER(?role = ' + '<' + base64.decode(req.params.roleURI) + '>)'
	)
    }

    if(req.params.igemResultsURI) {
	criteria.push(
	    '   ?subject igem:results ?results .' +
		'   FILTER(?results = ' + '<' + base64.decode(req.params.igemResultsURI) + '>)'
	)
    }

    if(req.params.igemStatusURI) {
	criteria.push(
	    '   ?subject igem:status ?status .' +
		'   FILTER(?status = ' + '<' + base64.decode(req.params.igemStatusURI) + '>)'
	)
    }
