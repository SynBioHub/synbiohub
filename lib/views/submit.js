...
const updateExplorerIndex = require('../updateExplorerIndex')
...
async function submitPlugin (pluginIdx, filename) {
  ...
  try {
    ...
    if (!pluginUp) {
      const err = new Error(`The submit plugin ${plugin.name} is not available. Please check the plugin configuration or contact the administrator.`)
      err.status = 500
      throw err
    }
    ...
    console.error('There was an error with submit plugin ' + plugin.name)
...
function submitPost (req, res) {
  ...
  // Guard against formidable firing 'close' more than once for a single
  // request, which would run handleSubmission twice on the same response and
  // crash with ERR_HTTP_HEADERS_SENT (and upload the submission twice).
  form.on('close', () => {
    if (req.submissionHandled) { return }
    req.submissionHandled = true
...
async function handleSubmission (req, res, submission) {
  ...
  try {
    ...
    if (req.forceNoHTML || !req.accepts('text/html')) {
      res.status(err.status || 500).type('text/plain').send(err.message || 'An error occurred while running the submit plugin')
      return
    } else {
  ...
  // Incrementally index the newly uploaded collection and its members in
  // SBOLExplorer so they are searchable immediately (no-op if disabled).
  // Fire-and-forget: never block or delay the submission response on indexing.
  updateExplorerIndex(collectionURI, submissionData.createdBy.graphUri)
