	...
	function convertSBOL(next) {
		...
		{ method: 'POST',
		  ...
		  json: { 'options': {'language' : 'SBOL2',
				      ...
				      'subset_uri': '',
				      'uri_prefix': config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.email) + '/',
				      'version': '1',
				      'insert_type': false,
			...
			res.status(response.statusCode).send(err)
 	   		...
 	   		console.log('http://www.async.ece.utah.edu/' + body.output_file);
			request({
                            method: 'GET',
 	   		    uri: 'http://www.async.ece.utah.edu/' + body.output_file,
			},
				function(err, response, body) {
				    if(err || response.statusCode >= 300) {
					console.log("err=" + err);
        ...
        function saveSubmission(next) {
            ...
            collection.uri = config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.email) + '/' + submissionData.id + '/1'
            collection.persistentIdentity = config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.email) + '/' + submissionData.id
	    collection.displayId = submissionData.id
