                    ...
                    return sparql.uploadFile(submissionData.createdBy.graphUri, resultFilename, 'application/rdf+xml').then((result) => {
                        ...
                        return sparql.queryJson(query, submissionData.createdBy.graphUri).then(models => {
                            ...
                            return Promise.all(models.map(model => {
                                ...
                                let sourceFilename = sourceUri.replace("file://", "")
                                ...
                                console.log(sbmlFiles)

                                let source = sbmlFiles.find(file => {
                                    console.log("file: " + file)
                                    file = file.replace("unzipped/", "")
                                    console.log("file: " + file)

                                    return sourceFilename.endsWith(file)
                                });
                                ...
                                let replacementQueryParameters = {
                                    modelUri: modelUri,
                                    attachmentUri: uploadedAttachments[source.replace("unzipped/", "")]
                                }
