                ...
                return saveTempFile().then((tmpFilename) => {
                    ...
                    return sparql.uploadFile(submissionData.createdBy.graphUri, resultFilename, 'application/rdf+xml').then((result) => {
                        ...
                        attachmentFiles.forEach(attachment => {
                            ...
                            uploads.createUpload(fileStream).then((uploadInfo) => {
                                const { hash, size } = uploadInfo
                                attachment = attachment.replace('unzipped/', '')
                                ...
                                return attachments.addAttachmentToTopLevel(
                                    ...
                                    submissionData.createdBy.username).then(attachmentUri => {
                                        uploadedAttachments[attachment] = attachmentUri;
                                    })
                            }).then(result => {
                                return fs.unlink(attachment);
                            })
                        })
                    }).then(() => {
                        let collectionUri = config.get('databasePrefix') + 'user/' + encodeURIComponent(submissionData.createdBy.username) + '/' + collectionId + '/' + collectionId + '_collection/' + version;
                        let query = loadTemplate('./sparql/GetModels.sparql', {collectionUri: collectionUri});
                        
                        return sparql.queryJson(query, submissionData.createdBy.graphUri).then(models => {
                            return Promise.all(models.map(model => {
                                let modelUri = model.modelUri;
                                let sourceUri = model.source;
                                let sourceFilename = sourceUri.replace("file://", "").replace()

                                if(sourceUri.indexOf("://") != -1) {
                                    return;
                                }

                                let source = sbmlFiles.find(file => sourceFilename.endsWith(file));

                                if(source.length == 0) {
                                    return Promise.resolve()
                                }

                                let replacementQueryParameters = {
                                    modelUri: modelUri,
                                    attachmentUri: uploadedAttachments[source]
                                }

                                let query = loadTemplate("sparql/AttachmentUpdate.sparql", replacementQueryParameters);
