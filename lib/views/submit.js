...
//    }
	...
	function convertSBOL(next) {
        ...
        function saveSubmission(next) {
            ...
            collection.uri = config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.email) + '/' + submissionData.id + '_collection/1'
            collection.persistentIdentity = config.get('databasePrefix') + 'user/' + encodeURIComponent(req.user.email) + '/' + submissionData.id + '_collection'
