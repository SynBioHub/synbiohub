...
function submitPost(req, res) {
    ...
    form.parse(req, (err, fields, files) => {
	    ...
	    collectionChoices = fields.collectionChoices
            if (req.forceNoHTML || !req.accepts('text/html')) {
		if (fields.collectionChoices[0]) {
		    collectionChoices = fields.collectionChoices[0].split(',')
		}
	    } else if (collectionChoices) {
		collectionChoices.forEach((collectionChoice) => {
		    collectionChoice = config.get(databasePrefix) + "public/" + collectionChoice
		})
	    }
