    ...
    form.parse(req, (err, fields, files) => {
		...
		if (!collectionUri || collectionUri === '') {
		    ...
		    collectionId = metaData.displayId.replace('_collection', '')
                    ...
                    var removeQuery = loadTemplate('sparql/removeSubmission.sparql', templateParams)
			...
			overwrite_merge: overwrite_merge
                    ...
                    const { success, log, errorLog, resultFilename, attachmentFiles } = result
                            ...
                            attachmentFiles.forEach(attachment => {
                                ...
                                uploads.createUpload(fileStream).then((uploadInfo) => {
                                    ...
                                    return attachments.addAttachmentToTopLevel(
                                        ...
                                        config.get('databasePrefix') + 'user/' + encodeURIComponent(submissionData.createdBy.username) + '/' + collectionId + '_collection/1/',
                                        attachment, 
                                        hash,
                                        size,
