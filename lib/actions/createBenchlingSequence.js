...
var sbolmeta = require('../views/utils/sbolmeta')
...
exports = module.exports = function(req, res) {
	...
	if(req.method === 'GET') {
		...
		Object.keys(config.get('remotes')).filter(function(e){return config.get('remotes')[e].type ==='benchling' &&
 config.get('remotes')[e].public}).forEach((remote) => {
	...
	var meta = sbolmeta.summarizeComponentDefinition(componentDefinition,req)
	...
	return benchling.createSequence(remoteConfig, bSeq).then((result) => {
	    ...
	    res.redirect('/public/' + remoteConfig.id + '/' + result.id /*remoteConfig.folderPrefix + bSeq.folder*/ + '/current')
	})
