  ...
  function makePublic () {
    ...
    return saveTempFile().then((tmpFilename) => {
      ...
      return sparql.uploadFile(null, resultFilename, 'application/rdf+xml').then(function removeSubmission (next) {
        // Incrementally index the now-public collection and its members in
        // SBOLExplorer so they are searchable immediately (no-op if disabled).
        updateExplorerIndex(collectionUri, null)
        ...
        if (req.params.version !== 'current') {
          ...
          return sparql.deleteStaggered(removeQuery, graphUri).then(() => {
            // The private original was just removed from the triplestore, but
            // SBOLExplorer's index still holds its entries. Tell SBOLExplorer to
            // drop them so the now-removed private collection stops appearing in
            // that user's search (no-op when SBOLExplorer is disabled).
            // Fire-and-forget: never block the make-public response on indexing.
