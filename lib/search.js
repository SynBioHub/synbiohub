...
search.lucene = function lucene (value) {
  ...
  // For sequence search
  if (values[0].includes('sequence') || values[0].includes('file_search')) {
    for (i = 0; i < values.length - 1; i++) {
      var sequenceQuery = values[i].split('=')

      if (sequenceQuery[0].includes('sequence')) {
        criteriaStr += `?subject sbol2:sequence ?seq .\n    ?seq sbol2:elements "${sequenceQuery[1]}" .\n`
        if (sequenceQuery[0] === 'sequence') { criteriaStr += ` # flag_search_exact: True` }
      } else if (sequenceQuery[0].includes('file_search')) {
        criteriaStr += `  # flag_file_search: ${decodeURIComponent(sequenceQuery[1])}\n`
      } else if (i > 0) {
        criteriaStr += `  # flag_${sequenceQuery[0]}: ${sequenceQuery[1]}\n`
      }
    }
  } else {
    for (i = 0; i < values.length - 1; i++) {
      ...
      if (query[0].indexOf(':') > -1) {
        ...
        criteriaStr += query[1] + ' sbol2:member ?subject .'
