                ...
                if(err) {
                    ...
                    callback(null, count, results.map((result) => {
                        ...
                        if(prefixedUri.prefix)
                            result.url = '/entry/' + prefixedUri.prefix + '/' + prefixedUri.uri
                        else
                            result.url = '/entry/' + result.base64Uri
...
search.lucene = function lucene(predicate, value) {
    ...
    return sparql.escape(
        '?subject <' + searchPrefix + 'matches> [' +
            '<' + searchPrefix + 'property> ' + property.predicate + ';' +
            '<' + searchPrefix + 'query> %L ]',
                opts.value
    ) + ' .'
