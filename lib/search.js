...
function search(graphUri, criteria, offset, limit, user, explorer = false) {
    ...
    return Promise.all([
		...
		if (result.uri.toString().startsWith(config.get('databasePrefix'))) {
                ...
                result.triplestore = result.uri.toString().startsWith(config.get('databasePrefix')+'public/')?'public':result.uri.toString().startsWith(config.get('databasePrefix')+'user/')?'private':'remote'
