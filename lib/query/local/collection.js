...
function getCollectionMembers(uri, graphUri, limit, offset, sortColumn, search) {
	...
	sortColumn.column !== undefined) {
        if (sortColumn.column == 'name') {
            ...
            '(UCASE(concat(str(?name),str(?displayId)))) '
        } else if (sortColumn.column == 'type') {
            sort = ' ORDER BY ' + sortColumn.dir.toUpperCase() + '(UCASE(str(?type))) ' +
            sortColumn.dir.toUpperCase() + '(UCASE(concat(str(?name),str(?displayId)))) '
        } else {
            sort = ' ORDER BY ' + sortColumn.dir.toUpperCase() + '(UCASE(str(?' + sortColumn.column + '))) '
        }
    } else {
        sort = ' ORDER BY ASC(UCASE(str(?type))) ASC(UCASE(concat(str(?name),str(?displayId)))) '
    }
