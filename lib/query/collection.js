...
function getCollectionMemberCount (uri, graphUri, search, typeFilter) {
  ...
  if (remoteConfig !== undefined && version === 'current') {
    ...
    return local.getCollectionMemberCount(uri, graphUri, search, typeFilter)
  }
}
...
function getCollectionMembers (uri, graphUri, limit, offset, sort, filter, typeFilter) {
  const { submissionId, version } = splitUri(uri)
  const remoteConfig = config.get('remotes')[submissionId]

  if (remoteConfig !== undefined && version === 'current') {
    return remote[remoteConfig.type].getCollectionMembers(remoteConfig, uri, limit, offset, sort, filter)
  } else {
    return local.getCollectionMembers(uri, graphUri, limit, offset, sort, filter, typeFilter)
  }
}
