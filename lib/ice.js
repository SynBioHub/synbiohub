...
function postIceFile(remoteConfig, path, sbol) {
    ...
    function attempt() {
		...
		formData: { "file": serializeSBOL(sbol).toString('utf8') },
		rejectUnauthorized: remoteConfig['rejectUnauthorized'],
                url: remoteConfig.url + path
...
function postIceJson(remoteConfig, path) {
...
function createSequence(remoteConfig, sbol) {

    return postIceFile(remoteConfig, '/rest/file/sequence/', sbol).then((result) => {
	var entryId = parseInt(result.entryId)
	return postIceJson(remoteConfig, '/rest/parts/' + entryId + '/permissions/')
    })
